    ...
    <section className="py-16 bg-gradient-to-br from-warm-50 to-primary-50/20">
      <div className="container-padding max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          {subtitle && <p className="text-secondary-600 text-lg leading-relaxed">{subtitle}</p>}
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {section.phones.map((phone, i) => (
            <div key={i} className="flex items-start gap-4 bg-white rounded-xl p-5 shadow-sm border border-secondary-100">
              <div className="w-10 h-10 rounded-lg bg-primary-50 flex items-center justify-center shrink-0">
                <Phone className="w-5 h-5 text-primary-600" />
              </div>
              <div>
                <p className="text-sm text-secondary-500 mb-0.5">{getLocalizedField(phone.label, locale)}</p>
          ...
          <div className="flex items-start gap-4 bg-white rounded-xl p-5 shadow-sm border border-secondary-100">
            <div className="w-10 h-10 rounded-lg bg-primary-50 flex items-center justify-center shrink-0">
              <Mail className="w-5 h-5 text-primary-600" />
            </div>
            <div>
              <p className="text-sm text-secondary-500 mb-0.5">Email</p>
